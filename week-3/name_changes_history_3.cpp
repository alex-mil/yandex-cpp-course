...
vector<string> FindNamesHistory_3(const map<int, string> &names_by_year, int year) {
  vector<string> names;
...
const string BuildJoinedName_3(vector<string> names) {
...
string BuildFullName_3(const string &first_name, const string &last_name) {
...
class Person {
 public:
  ...
  string GetFullName(int year) const {
    ...
    // в данном случае это излишне, так как нам достаточно узнать последние имя и фамилию,
    // но почему бы не использовать готовые функции?
    ...
    // подготовим данные для функции BuildFullName_3: возьмём последние имя и фамилию или оставим
    // их пустыми, если они неизвестны
    string first_name;
    string last_name;
    ...
    return BuildFullName_3(first_name, last_name);
  }
  ...
  string GetFullNameWithHistory(int year) const {
    ...
    const string first_name = BuildJoinedName_3(FindFirstNamesHistory(year));
    // получим полную фамилию со всей историей
    const string last_name = BuildJoinedName_3(FindLastNamesHistory(year));
 ...
 private:
  ...
  vector<string> FindFirstNamesHistory(int year) const {
    return FindNamesHistory_3(first_names, year);
  }

  vector<string> FindLastNamesHistory(int year) const {
    return FindNamesHistory_3(last_names, year);
  }
};